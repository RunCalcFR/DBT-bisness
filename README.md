...
DBT bisness est un système d'exploitation business propulsé par l'IA pour les entrepreneurs. Clone ce repo, ouvre-le dans Cursor, et obtiens un associé business qui connaît ton business, identifie tes blocages, et te guide vers ton prochain palier — chaque jour.

## Ce que DBT bisness fait

- **Première fois :** DBT bisness te pose des questions pour comprendre ton business, diagnostique tes problèmes principaux, et t'aide immédiatement à passer à l'action.
- **À chaque retour :** Reviens et parle. DBT bisness se souvient de tout, se met à jour sur ce qui s'est passé, et propose la tâche au plus grand levier. C'est juste une conversation avec un associé business qui connaît ta situation.

### 10 skills spécialisés

DBT bisness n'est pas un assistant générique — c'est une équipe de spécialistes, chacun avec sa propre méthodologie :
...
Tu n'as jamais besoin d'« activer » un skill. Parle — DBT bisness détecte ce dont tu as besoin et applique le bon spécialiste.
...
DBT bisness vit dans ton workspace Cursor sous forme de fichiers markdown + 10 skills IA spécialisés :
...
### 2. Cloner ce repo
```bash
git clone https://runcalcfr.github.io/DBT-bisness/
cd DBT bisness
```
...
### 4. Parler
Ouvre une nouvelle conversation IA dans Cursor et dis n'importe quoi.
DBT bisness prend les rênes.
